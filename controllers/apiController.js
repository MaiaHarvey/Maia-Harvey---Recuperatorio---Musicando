...
module.exports = {
    canciones: async (req, res) => {
        try {
            ...
            if (canciones.length == 0) {
                return res.json({ mensaje: 'No se encontraron canciones'})
            }
            res.json(canciones)
        } catch (error) {
            console.log(error)
    ...
    update: async (req, res) => {
        ...
        try {
            ...
            console.log(datosActualizados + 'datos actualizados')
    ...
    delete: async (req, res) => {
        ...
        if (cancionBorada) {
            ...
            res.json({ mesaje: "La cancion con el Id" + id + "no existe" });
        }
    },
    ...
    getList: async (req, res) => {
        try {
            ...
            if (generos.length == 0) {
                return res.json({ mensaje: 'No se encontraron generos'});
            }
            res.json(generos);
